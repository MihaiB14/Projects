...
function [h, pr] = firls_FTS_c(M,w_p,w_s,K)
...
% ~~~~~~~~~~~~~
	...
	R = w_s*sinc(M*w_s) - w_p*sinc(M*w_p) + eye(size(M));
	R = toeplitz(R);		% This is the matrix to invert.
	r = sinc(K - M) - w_s*sinc((K-M)*w_s);	% This is the free vector.
	h = R\r;
    pr = 100*((pi - w_s - r'*h)/(pi - w_s));
%
% END
%
